  ...
  return (
    <Link href={href} className="group block min-w-0">
      <div className="aspect-[3/4] relative overflow-hidden bg-[#f0eeeb] mb-4">
        ...
        {badge && (
          <div className="absolute bottom-3 left-3 bg-white/90 px-2.5 py-1 text-[12px] tracking-[0.08em] uppercase text-[#0a0a0a]">
            {badge}
          </div>
        )}
      </div>
      {kicker && (
        <span className="text-[12px] text-[#0a0a0a]/70 tracking-[0.1em] uppercase block mb-1">
          {kicker}
        </span>
      )}
      ...
      {sub && <p className="text-[12px] text-[#0a0a0a]/70 mt-1 mb-0 leading-relaxed">{sub}</p>}
    </Link>
  );
}
